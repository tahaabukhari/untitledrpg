...
| `weapons/behaviors/*.gd` | `WeaponBehavior` (base = melee) + `laser_behavior`, `heal_behavior`, `prayer_behavior`. Own charged-attack dispatch (`on_release`, `wants_charge_stance`); the player forwards charge events here instead of branching on a style enum. |
| Healer "Prayer" | `healer_prayer.tres` (empty hands, healer default; wand rides in inventory) + `prayer_animator.gd` (hand-rub anim, method track) + `prayer_behavior.gd`. Every release prays; 1-in-7 → `Fx.lightning_strike` on the closest enemy ≤520px: full `charged_damage` direct + 35% light AoE ≤90px + burning embers. Player verbs: `perform_prayer`/`on_prayer_completed`/`_closest_enemy`; puppet `play_named_attack(anim)` plays weapon anims outside the combo/aim pipeline. |
| Directional dodge | `_perform_dodge_roll` picks a variant from movement input: up→leap, down→**slide**, else→roll (`DodgeVariant` enum). Puppet `_make_slide`/`_make_knockdown`. See docs/COMBAT_DODGE_TRIP_MIRROR.md. |
| Tripping (bipeds only) | Slide-under → `trip()` → DOWNED 2s (on knees, bonus damage, no input). Symmetric: player `trip()`/`is_downed`/`stagger()` + `EnemyBase.trippable`/`trip()`/`_on_tripped`. `Fx.trip_dust`. |
| Blood | `Fx.blood_spray` (gory) routed by `EnemyBase.hit_fx` ("flesh" bleeds, "goo" default). Only the Mirror is flesh for now. |
| `enemies/mirror_warrior.gd/.tscn` | Glowing-blue player reflection — reuses the player puppet (`enemies/mirror_skin.tscn`, a standalone copy of PlayerSkin). Warrior sword combos (shared swing builder + EnemyBase hitbox), parry (negates + `player.stagger()` + riposte), evasive roll (i-frames), slide-trip. Bipedal + flesh. Arena spawn: **M** (`dbg_spawn_mirror`). |
